...
final class EditorTextView: NSTextView, CurrentLineHighlighting, MultiCursorEditing {
    ...
    var mode: ModeOptions = .init() { didSet { if mode != oldValue { applyMode() } } }
    ...
    var commentDelimiters: Syntax.Comment = .init()
    var syntaxCompletionWords: [String] = []

    var needsUpdateLineHighlight = true {
        didSet {
            (lineHighlightRects + [visibleRect]).forEach { self.setNeedsDisplay($0, avoidAdditionalLayout: true) }
        }
    }
    ...
    var insertionLocations: [Int] = [] { didSet { needsUpdateInsertionIndicators = true } }
    ...
    private var isAutomaticPeriodSubstitutionEnabled = false { didSet { invalidateAutomaticPeriodSubstitution() } }
    ...
    required init(textStorage: NSTextStorage, lineEndingScanner: LineEndingScanner) {
        ...
        identifier = NSUserInterfaceItemIdentifier("EditorTextView")
        ...
        allowsUndo = true
        isRichText = false
        baseWritingDirection = .leftToRight // default is fixed in LTR
        linkTextAttributes?.removeValue(forKey: .foregroundColor)
        ...
        layoutManager.tabWidth = tabWidth

        // setup behaviors
        isAutomaticTabExpansionEnabled = defaults[.autoExpandTab]
        isAutomaticIndentEnabled = defaults[.autoIndent]
...
//        coder.encode(font, forKey: SerializationKey.font)
//        coder.encode(Double(scale), forKey: SerializationKey.scale)
//        coder.encode(tabWidth, forKey: SerializationKey.tabWidth)
//        coder.encode(insertionLocations, forKey: SerializationKey.insertionLocations)
    }
...
//        }
    ...
    override func viewDidMoveToWindow() {
        ...
        if let window = window as? DocumentWindow, let theme = theme {
            window.contentBackgroundColor = theme.background.color
        }

        // apply window opacity
        windowOpacityObserver = window?.publisher(for: \.isOpaque, options: .initial)
            .sink { [weak self] in
        ...
        if let window {
            ...
            instanceHighlightTask?.cancel()
            instanceHighlightTask = nil
        }
    }
    ...
    override func setFrameSize(_ newSize: NSSize) {
        ...
        if !inLiveResize {
            overscrollResizingDebouncer.schedule()
        }

        needsUpdateInsertionIndicators = true
        needsUpdateLineHighlight = true
...
//        super.updateTextTouchBarItems()
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        isPerformingRectangularSelection = event.modifierFlags.contains(.option)
        needsUpdateInsertionIndicators = true // to draw dummy indicator for proper one while selecting
...
//        }
    ...
    override func cancelOperation(_ sender: Any?) {
        ...
        if hasMultipleInsertions {
            selectedRange = insertionRanges.first!
    ...
    override func didChangeText() {
        ...
        if UserDefaults.standard[.autoTrimsTrailingWhitespace] {
            trimTrailingWhitespaceTask.schedule(delay: .seconds(3))
        }

        // retry completion if needed
        // -> Flag is set in `insertCompletion(_:forPartialWordRange:movement:isFinal:)`.
        if needsRecompletion {
            completionDebouncer.schedule(delay: .milliseconds(50))
        }
    }
    ...
    override func insertText(_ string: Any, replacementRange: NSRange) {
        ...
        if insertionRanges.count > 1 {
            insertText(plainString, replacementRanges: insertionRanges)
            return
        }

        // balance brackets and quotes
        if isAutomaticSymbolBalancingEnabled, replacementRange.isEmpty {
            // with opening symbol input
            if let pair = matchingBracketPairs.first(where: { String($0.begin) == plainString }) {
                // wrap selection with brackets if some text is selected
                if !rangeForUserTextChange.isEmpty {
                    surroundSelections(begin: String(pair.begin), end: String(pair.end))
                    return
                }

                // insert bracket pair if insertion point is not in a word
                if !CharacterSet.alphanumerics.contains(character(after: rangeForUserTextChange) ?? Unicode.Scalar(0)),
                   !(pair.begin == pair.end && CharacterSet.alphanumerics.contains(character(before: rangeForUserTextChange) ?? Unicode.Scalar(0))) // for "
                {
                    // raise frag to manipulate the cursor later in `handleTextCheckingResults(_:forRange:types:options:orthography:wordCount:)`
                    if isAutomaticQuoteSubstitutionEnabled, pair.begin == "\"" {
                        isTypingPairedQuotes = true
                    }

                    super.insertText(String(pair.begin) + String(pair.end), replacementRange: replacementRange)
                    setSelectedRangesWithUndo([NSRange(location: selectedRange.location - 1, length: 0)])
                    textStorage?.addAttribute(
                        .autoBalancedClosingBracket,
                        value: true,
                        range: NSRange(location: selectedRange.location, length: 1)
                    )
                    return
                }
            }
            ...
            if matchingBracketPairs.contains(where: { String($0.end) == plainString }),
               plainString.unicodeScalars.first == character(after: rangeForUserTextChange),
               textStorage?.attribute(.autoBalancedClosingBracket, at: selectedRange.location, effectiveRange: nil) as? Bool == true
            {
                textStorage?.removeAttribute(.autoBalancedClosingBracket, range: NSRange(location: selectedRange.location, length: 1))
        ...
        // smart outdent with '}'
        if isAutomaticIndentEnabled, replacementRange.isEmpty,
           plainString == "}"
        {
            let insertionIndex = String.Index(utf16Offset: rangeForUserTextChange.upperBound, in: self.string)
            ...
            {
                let desiredLevel = self.string.indentLevel(at: precedingIndex, tabWidth: tabWidth)
                let currentLevel = self.string.indentLevel(at: insertionIndex, tabWidth: tabWidth)
        ...
        if isAutomaticCompletionEnabled {
            if rangeForUserCompletion.length >= UserDefaults.standard[.minimumAutomaticCompletionLength] {
                let delay: TimeInterval = UserDefaults.standard[.autoCompletionDelay]
                completionDebouncer.schedule(delay: .seconds(delay))
            } else {
    ...
    override func insertTab(_ sender: Any?) {
        // indent with tab key
        if UserDefaults.standard[.indentWithTabKey], !rangeForUserTextChange.isEmpty {
            indent()
            return
        }

        // insert soft tab
        if isAutomaticTabExpansionEnabled {
            let insertionRanges = rangesForUserTextChange?.map(\.rangeValue) ?? [rangeForUserTextChange]
            let softTabs = insertionRanges
                .map { self.string.softTab(at: $0.location, tabWidth: self.tabWidth) }

            replace(with: softTabs, ranges: insertionRanges, selectedRanges: nil)
            return
        }
    ...
    override func insertNewline(_ sender: Any?) {
        guard isEditable,
              isAutomaticIndentEnabled
        else { return insertText(lineEnding.string, replacementRange: rangeForUserTextChange) }

        let tab = isAutomaticTabExpansionEnabled ? String(repeating: " ", count: tabWidth) : "\t"
        let ranges = rangesForUserTextChange?.map(\.rangeValue) ?? [rangeForUserTextChange]

        let indents: [(range: NSRange, indent: String, insertion: Int)] = ranges
            .map { range in
                guard let indentRange = self.string.rangeOfIndent(at: range.location),
                      !indentRange.isEmpty,
        ...
        // insert newline
        insertText(lineEnding.string, replacementRange: rangeForUserTextChange)

        // auto indent
        var locations: [Int] = []
        var offset = 0
        for (range, indent, insertion) in indents {
            let location = range.lowerBound + lineEnding.length + offset

            super.insertText(indent, replacementRange: NSRange(location: location, length: 0))

            offset += -range.length + lineEnding.length + indent.count
            locations.append(location + insertion)
        }
        setSelectedRangesWithUndo(locations.map { NSRange(location: $0, length: 0) })
    }

    override func deleteBackward(_ sender: Any?) {
        guard isEditable else { return super.deleteBackward(sender) }

        if multipleDelete() { return }

        // delete tab
        if isAutomaticTabExpansionEnabled,
           let deletionRange = string.rangeForSoftTabDeletion(in: rangeForUserTextChange, tabWidth: tabWidth)
        {
            setSelectedRangesWithUndo(selectedRanges.map(\.rangeValue))
            selectedRange = deletionRange
        }

        // balance brackets
        if isAutomaticSymbolBalancingEnabled,
           rangeForUserTextChange.isEmpty,
           let lastCharacter = character(before: rangeForUserTextChange),
           let nextCharacter = character(after: rangeForUserTextChange),
           matchingBracketPairs.contains(where: { $0.begin == Character(lastCharacter) && $0.end == Character(nextCharacter) })
        {
            setSelectedRangesWithUndo(selectedRanges.map(\.rangeValue))
            selectedRange = NSRange(location: rangeForUserTextChange.location - 1, length: 2)
        }
    ...
    override func cut(_ sender: Any?) {
        let insertionRanges = self.insertionRanges
        setSelectedRangesWithUndo(insertionRanges)
        ...
        guard let set = prepareForSelectionUpdate(ranges) else { return }

        selectedRanges = set.selectedRanges
        insertionLocations = set.insertionLocations
    }
    ...
    override func setSelectedRanges(_ ranges: [NSValue], affinity: NSSelectionAffinity, stillSelecting stillSelectingFlag: Bool) {
        ...
        if isPerformingRectangularSelection {
            if let locations = insertionLocations(from: mouseDownPoint, candidates: ranges, affinity: affinity) {
                ranges = [NSRange(location: locations[0], length: 0)] as [NSValue]
                insertionLocations = Array(locations[1...])
            } else {
                insertionLocations = []
            }
        }

        let currentRanges = rangesForUserTextChange ?? self.selectedRanges
        ...
        insertionLocations.removeAll { location in selectedRanges.contains { $0.touches(location) } }

        if !stillSelectingFlag, !hasMultipleInsertions {
            selectionOrigins = [selectedRange.location]
        }
        ...
        if UserDefaults.standard[.highlightSelectionInstance] {
            instanceHighlightTask?.cancel()
            if let layoutManager = layoutManager, layoutManager.hasTemporaryAttribute(.roundedBackgroundColor) {
                layoutManager.removeTemporaryAttribute(.roundedBackgroundColor, forCharacterRange: string.range)
            }
        }

        if !stillSelectingFlag, !isShowingCompletion {
            // highlight matching brace
            if UserDefaults.standard[.highlightBraces] {
                let bracePairs = BracePair.braces + (UserDefaults.standard[.highlightLtGt] ? [.ltgt] : [])
                highlightMatchingBrace(candidates: bracePairs, in: visibleRect)
            }

            // update instances highlight
            if UserDefaults.standard[.highlightSelectionInstance] {
                highlightInstances(after: .seconds(UserDefaults.standard[.selectionInstanceHighlightDelay]))
            }
        }
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        if copyIndex >= 0 { // -1 == not found
            menu.insertItem(
                withTitle: String(localized: "Copy as Rich Text", table: "MainMenu"),
        ...
        if pasteIndex >= 0 { // -1 == not found
            menu.insertItem(
                withTitle: String(localized: "Select All", table: "MainMenu"),
        ...
        for item in menu.items {
            ...
            submenu.insertItem(
                withTitle: String(localized: "Straighten Quotes", table: "MainMenu"),
    ...
    override var typingAttributes: [NSAttributedString.Key: Any] {
        didSet {
            ...
            guard hasMarkedText() != true else { return }

            (textContainer as? TextContainer)?.indentAttributes = typingAttributes
        }
    }
    ...
    override var font: NSFont? {
        get {
            ...
            (layoutManager as? LayoutManager)?.textFont ?? super.font
        }

        set {
            guard let font = newValue else { return }

            willChangeValue(for: \.font)
            defer {
            ...
            if font.isFixedPitch {
                ...
                textStorage?.addAttribute(.kern, value: 0, range: string.range)
            } else {
                typingAttributes[.kern] = nil
                textStorage?.removeAttribute(.kern, range: string.range)
            }
            ...
            (layoutManager as? LayoutManager)?.textFont = font

            invalidateDefaultParagraphStyle()
            needsUpdateLineHighlight = true
            needsUpdateInsertionIndicators = true
    ...
    override func viewWillDraw() {
        ...
        if needsUpdateInsertionIndicators {
            updateInsertionIndicators()
            needsUpdateInsertionIndicators = false
        }
    }
    ...
    override func drawBackground(in rect: NSRect) {
        ...
        guard let range = range(for: rect) else { return }

        // draw current line highlight
        if UserDefaults.standard[.highlightCurrentLine] {
            drawCurrentLine(range: range, in: rect)
        }

        drawRoundedBackground(range: range, in: rect)
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        // draw page guide
        if showsPageGuide,
           let spaceWidth = (layoutManager as? LayoutManager)?.spaceWidth
        {
            ...
            let isRTL = (baseWritingDirection == .rightToLeft)

            let guideRect = NSRect(
                x: isRTL ? bounds.width - x : x,
                y: dirtyRect.minY,
                width: 1.0,
                height: dirtyRect.height
            )

            if guideRect.intersects(dirtyRect), let textColor = textColor {
                ...
                guideColor.setFill()
                centerScanRect(guideRect).intersection(dirtyRect).fill()
    ...
    override func scrollRangeToVisible(_ range: NSRange) {
        ...
        guard NSEvent.modifierFlags.contains(.numericPad),
              range.upperBound < (layoutManager?.firstUnlaidCharacterIndex() ?? 0),
    ...
    override func setLayoutOrientation(_ orientation: NSLayoutManager.TextLayoutOrientation) {
        let didChange = orientation != layoutOrientation

        // -> Need to send KVO notification manually on Swift. (2016-09-12 on macOS 10.12 SDK)
        willChangeValue(for: \.layoutOrientation)
        super.setLayoutOrientation(orientation)
        didChangeValue(for: \.layoutOrientation)
        ...
        if wrapsLines {
            ...
            frame.size[keyPath: keyPath] = visibleRect.width * scale
        }
    ...
    override func readSelection(from pboard: NSPasteboard, type: NSPasteboard.PasteboardType) -> Bool {
        ...
        if pboard.name == .general,
           ...
           let ranges = rangesForUserTextChange?.map(\.rangeValue),
           ranges.count > 1,
           string.rangeOfCharacter(from: .newlines) == nil
        {
            return insertText(string, replacementRanges: ranges)
        }
           ...
           ranges.count > 1
        {
            ...
            return replace(with: strings, ranges: ranges, selectedRanges: nil)
        }
    ...
    override var baseWritingDirection: NSWritingDirection {
        willSet {
            willChangeValue(for: \.baseWritingDirection)
        }
        ...
        didSet {
            ...
            didChangeValue(for: \.baseWritingDirection)
    ...
    override func validateUserInterfaceItem(_ item: any NSValidatedUserInterfaceItem) -> Bool {
        ...
        case #selector(straightenQuotesInSelection):
            ...
            return isEditable && !selectedRange.isEmpty

        case #selector(toggleComment):
            (item as? NSMenuItem)?.title = canUncomment(partly: false)
                ? String(localized: "Uncomment", table: "MainMenu")
                : String(localized: "Comment Out", table: "MainMenu")
            return isEditable && ((commentDelimiters.inline != nil) || (commentDelimiters.block != nil))

        case #selector(commentOut):
            return isEditable

        case #selector(inlineCommentOut):
            return isEditable && (commentDelimiters.inline != nil)

        case #selector(blockCommentOut):
            return isEditable && (commentDelimiters.block != nil)

        case #selector(uncomment(_:)):
            return isEditable && canUncomment(partly: true)
    ...
    var tabWidth: Int = 4 {
        didSet {
            ...
            (layoutManager as? LayoutManager)?.tabWidth = tabWidth
    ...
    var showsPageGuide = false {
        didSet {
            setNeedsDisplay(frame, avoidAdditionalLayout: true)
        }
    }
    ...
    var showsIndentGuides: Bool {
        get {
            (layoutManager as? LayoutManager)?.showsIndentGuides ?? true
        }

        set {
            (layoutManager as? LayoutManager)?.showsIndentGuides = newValue
            setNeedsDisplay(frame, avoidAdditionalLayout: true)
        }
    }
    ...
    var usesAntialias: Bool {
        get {
            (layoutManager as? LayoutManager)?.usesAntialias ?? true
        }

        set {
            guard newValue != usesAntialias else { return }
            (layoutManager as? LayoutManager)?.usesAntialias = newValue
    ...
    var showsInvisibles: Bool {
        get {
            (layoutManager as? LayoutManager)?.showsInvisibles ?? false
        }

        set {
            (layoutManager as? LayoutManager)?.showsInvisibles = newValue
            needsUpdateInsertionIndicators = true
        }
    }
    ...
    func setFont(type: FontType) {
        let defaults = UserDefaults.standard
        font = defaults.font(for: type)
        ligature = defaults[.ligature(for: type)] ? .standard : .none
        usesAntialias = defaults[.antialias(for: type)]
    ...
    @IBAction func copyWithStyle(_ sender: Any?) {
        guard !selectedRange.isEmpty else { return NSSound.beep() }
        ...
        pboard.declareTypes([.rtf] + writablePasteboardTypes, owner: nil)
    ...
    @IBAction func pasteAsIs(_ sender: Any?) {
        isApprovedTextChange = true

        super.pasteAsPlainText(sender)

        isApprovedTextChange = false
    }
    ...
    @IBAction func selectEnclosingSymbols(_ sender: Any?) {
        guard let selectedRange = Range(selectedRange, in: string),
              let enclosingRange = string.rangeOfEnclosingBracePair(at: selectedRange, candidates: BracePair.braces + [.ltgt])
        else { return }

        self.selectedRange = NSRange(enclosingRange, in: string)
    }
    ...
    private func applyTheme() {
        ...
        assert(layoutManager != nil)
        assert(enclosingScrollView != nil)

        guard let theme = theme else { return assertionFailure() }

        textColor = theme.text.color
        backgroundColor = theme.background.color
        lineHighlightColor = theme.lineHighlight.color
        insertionPointColor = theme.insertionPointColor
        ...
        (layoutManager as? LayoutManager)?.invisiblesColor = theme.invisibles.color
        (layoutManager as? LayoutManager)?.unemphasizedSelectedContentBackgroundColor = theme.unemphasizedSelectionColor
        textHighlightColor = theme.highlightColor
        ...
        selectedTextAttributes[.foregroundColor] = nil

        (window as? DocumentWindow)?.contentBackgroundColor = theme.background.color
        enclosingScrollView?.backgroundColor = theme.background.color
        enclosingScrollView?.scrollerKnobStyle = theme.isDarkTheme ? .light : .default
    ...
    private func applyMode() {
        ...
        setFont(type: mode.fontType)

        smartInsertDeleteEnabled = mode.smartInsertDelete
        isAutomaticDashSubstitutionEnabled = mode.automaticDashSubstitution
        isAutomaticQuoteSubstitutionEnabled = mode.automaticQuoteSubstitution
        isAutomaticTextReplacementEnabled = mode.automaticTextReplacement
        isAutomaticPeriodSubstitutionEnabled = mode.automaticPeriodSubstitution
        isAutomaticSymbolBalancingEnabled = mode.automaticSymbolBalancing

        isContinuousSpellCheckingEnabled = mode.continuousSpellChecking
        isGrammarCheckingEnabled = mode.grammarChecking
        isAutomaticSpellingCorrectionEnabled = mode.automaticSpellingCorrection

        isAutomaticCompletionEnabled = mode.automaticCompletion && !mode.completionWordTypes.isEmpty
    }
    ...
    private func invalidateAutomaticPeriodSubstitution() {
        if window?.firstResponder == self {
            UserDefaults.standard[.automaticPeriodSubstitutionEnabled] = isAutomaticPeriodSubstitutionEnabled
        } else {
    ...
    private func invalidateDefaultParagraphStyle() {
        ...
        guard let paragraphStyle = defaultParagraphStyle?.mutable else { return assertionFailure() }
        ...
        if let font = font {
            ...
            paragraphStyle.defaultTabInterval = CGFloat(tabWidth) * font.width(of: " ")
        }
        ...
        guard paragraphStyle.lineHeightMultiple != defaultParagraphStyle?.lineHeightMultiple ||
            paragraphStyle.defaultTabInterval != defaultParagraphStyle?.defaultTabInterval
        else { return }

        defaultParagraphStyle = paragraphStyle
        typingAttributes[.paragraphStyle] = paragraphStyle
        textStorage?.addAttribute(.paragraphStyle, value: paragraphStyle, range: string.range)

        // tell line height also to scroll view so that scroll view can scroll line by line
        if let lineHeight = (layoutManager as? LayoutManager)?.lineHeight {
            enclosingScrollView?.lineScroll = lineHeight
        }
    ...
    private func invalidateOverscrollRate() {
        guard let layoutManager = layoutManager as? LayoutManager else { return }
        ...
        let diff = height - textContainerInset.height

        guard diff != 0 else { return }

        let heightPath = (layoutOrientation == .vertical) ? \NSSize.width : \NSSize.height
        textContainerInset.height = height
        frame.size[keyPath: heightPath] += 2 * diff
        ...
        guard let textContainer = textContainer else { return assertionFailure() }
    ...
    private func invalidateNonContiguousLayout() {
        layoutManager?.allowsNonContiguousLayout = if layoutOrientation == .vertical {
            ...
            string.length > UserDefaults.standard[.minimumLengthForNonContiguousLayout]
        }
    }
    ...
    private func highlightInstances(after delay: Duration) {
        instanceHighlightTask?.cancel()
        instanceHighlightTask = Task.detached {
            ...
            let (string, selectedRange): (String, NSRange) = try await MainActor.run {
                guard !self.string.isEmpty, // important to avoid crash after closing editor
                      !self.selectedRange.isEmpty,
                      !self.hasMarkedText(),
...
extension EditorTextView: TextFinderClient {
    ...
    override func performTextFinderAction(_ sender: Any?) {
        performEditorTextFinderAction(sender)
    }
    ...
    @IBAction func performEditorTextFinderAction(_ sender: Any?) {
        guard let tag = (sender as? any NSValidatedUserInterfaceItem)?.tag ?? (sender as? NSControl)?.tag,
              let action = TextFinder.Action(rawValue: tag)
        else { return }

        textFinder.performAction(action, representedItem: (sender as? NSMenuItem)?.representedObject)
    }
...
extension EditorTextView {
    ...
    override var rangeForUserCompletion: NSRange {
        ...
        guard !string.isEmpty else { return range }

        let firstSyntaxLetters = syntaxCompletionWords.compactMap(\.unicodeScalars.first)
        ...
        let invalidRange = (string as NSString).rangeOfCharacter(from: firstLetterSet.inverted, options: .backwards, range: searchRange)
    ...
    override func completions(forPartialWordRange charRange: NSRange, indexOfSelectedItem index: UnsafeMutablePointer<Int>) -> [String]? {
        ...
        if mode.completionWordTypes.contains(.document) {
        ...
        if mode.completionWordTypes.contains(.syntax) {
            let syntaxWords = syntaxCompletionWords.filter { $0.range(of: partialWord, options: [.caseInsensitive, .anchored]) != nil }
            candidateWords.append(contentsOf: syntaxWords)
        }

        // add the standard words from default completion words
        if mode.completionWordTypes.contains(.standard) {
    ...
    override func insertCompletion(_ word: String, forPartialWordRange charRange: NSRange, movement: Int, isFinal flag: Bool) {
        completionDebouncer.cancel()
        needsRecompletion = false

        isShowingCompletion = !flag

        // store original string
        if partialCompletionWord == nil {
            partialCompletionWord = (string as NSString).substring(with: charRange)
        }
        ...
        if flag, let event = window?.currentEvent, event.type == .keyDown, !event.modifierFlags.contains(.command),
        ...
        if flag {
            ...
            case NSIllegalTextMovement, NSRightTextMovement: // treat as cancelled
                ...
                if let originalWord = partialCompletionWord {
                    word = originalWord
                }
            default:
                didComplete = true
    ...
    private func performCompletion() {
        // abort if:
        guard !hasMarkedText(), // input is not specified (for Japanese input)
              selectedRange.isEmpty, // selected
              let lastCharacter = character(before: selectedRange), !CharacterSet.whitespacesAndNewlines.contains(lastCharacter) // previous character is blank
        else { return }

        if let nextCharacter = character(after: selectedRange), CharacterSet.alphanumerics.contains(nextCharacter) { return } // cursor is (probably) at the middle of a word
...
extension EditorTextView {
    ...
    override func selectionRange(forProposedRange proposedCharRange: NSRange, granularity: NSSelectionGranularity) -> NSRange {
        ...
        if !range.isEmpty {
            range = wordRange(at: proposedCharRange.location)
            if proposedCharRange.length > 1 {
                range.formUnion(wordRange(at: proposedCharRange.upperBound - 1))
            }
        }

        guard proposedCharRange.isEmpty, // not on expanding selection
              range.length == 1 // clicked character can be a brace
        else { return range }

        let characterIndex = String.Index(utf16Offset: range.lowerBound, in: string)
        let clickedCharacter = string[characterIndex]

        // select (syntax-highlighted) quoted text
        if ["\"", "'", "`"].contains(clickedCharacter),
           let highlightRange = layoutManager?.effectiveRange(of: .syntaxType, at: range.location)
        {
            let highlightCharacterRange = Range(highlightRange, in: string)!
            ...
            if (firstHighlightIndex == characterIndex && string[firstHighlightIndex] == clickedCharacter) || // begin quote
        ...
        if let pairRange = string.rangeOfBracePair(at: characterIndex, candidates: BracePair.braces + [.ltgt]) {
            return NSRange(pairRange, in: string)
        }
    ...
    func wordRange(at location: Int) -> NSRange {
        ...
        return (string as NSString).rangeOfCharacter(until: CharacterSet(charactersIn: ".:"), at: location, range: proposedWordRange)
    }
}
